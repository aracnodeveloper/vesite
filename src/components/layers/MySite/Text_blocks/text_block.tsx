// components/layers/MySite/Text_blocks/text_block.tsx

import { useEffect } from "react";
import { ChevronRight } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useTextBlocks } from "../../../../hooks/useTextBlocks";
import Cookies from "js-cookie";

const Text_blocks = () => {
    const navigate = useNavigate();
    const { blocks, getBlocksByBiosite } = useTextBlocks();

    useEffect(() => {
        const biositeId = Cookies.get('biositeId');
        if (biositeId) {
            getBlocksByBiosite(biositeId);
        }
    }, []);

    const handleGalleryClick = () => {
        navigate('/gallery');
    };

    // Contar imágenes reales (que tengan URL)
    const activeImagesCount = blocks.filter(b => b.image && b.isActive).length;

    return (
        <div
            className="bg-white/25 rounded-lg p-4 flex items-center justify-between cursor-pointer transition-colors hover:bg-white/35"
            onClick={handleGalleryClick}
        >
            <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-[#4A7BA8] rounded-lg flex items-center justify-center">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        viewBox="0 0 14 14"
                        fill="#ffffff"
                    >
                        <g fill="none" stroke="#ffffff" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M10.081 2.489a1.04 1.04 0 0 0-.93-.897c-1.155-.11-2.362-.247-3.603-.247s-2.448.138-3.604.247a1.04 1.04 0 0 0-.93.897C.89 3.417.75 4.383.75 5.375s.141 1.958.264 2.886c.063.478.45.852.93.897" />
                            <path d="M3.919 11.511c.063.478.45.852.93.897c1.155.11 2.362.247 3.603.247s2.448-.138 3.604-.247c.48-.045.867-.42.93-.897c.123-.928.264-1.894.264-2.886s-.141-1.958-.264-2.886a1.04 1.04 0 0 0-.93-.897c-1.156-.11-2.362-.247-3.604-.247c-1.24 0-2.448.138-3.603.247a1.04 1.04 0 0 0-.93.897c-.123.928-.264 1.894-.264 2.886s.14 1.958.264 2.886" />
                            <path d="M3.818 10.68a7.6 7.6 0 0 1 1.531-1.43c.566-.401 1.323-.432 1.889-.03c1.234.878 2.278 2.128 2.986 3.357m-.143-3.747c.784 0 1.224-.44 1.224-1.223s-.44-1.224-1.224-1.224c-.783 0-1.224.44-1.224 1.224c0 .783.441 1.224 1.224 1.224" />
                        </g>
                    </svg>
                </div>
                <span className="text-black font-medium">Galería</span>
            </div>

            <div className="flex items-center space-x-2">
                {activeImagesCount > 0 && (
                    <span className="text-sm text-black" style={{ fontSize: "11px" }}>
                        {activeImagesCount}
                    </span>
                )}
                <ChevronRight size={16} className="text-black" />
            </div>
        </div>
    );
};

export default Text_blocks;