...
const MusicPage = () => {
  ...
  const isValidUrl = (url: string) => {
    ...
    return musicPlatforms.some((platform) =>
      url.toLowerCase().includes(platform)
    );
  };

  const canSave =
    url.trim() !== "" && isValidUrl(url) && !isSubmitting && !isDeleting;
        ...
        <div>
          ...
          {url && !isValidUrl(url) && (
            <p className="text-red-400 text-xs mt-1">
              Please enter a valid URL from supported platforms (Spotify,
              SoundCloud, Apple Music, YouTube, etc.)
            </p>
          )}
        </div>
