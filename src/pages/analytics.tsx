...
const AnalyticsContent = () => {
  ...
  const generatePDFBlob = useCallback(async (): Promise<Blob> => {
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      if (activity.linkClicks && activity.linkClicks.length > 0) {
        doc.setFontSize(8);
        doc.setTextColor(...grayColor);
        const topLinks = activity.linkClicks.slice(0, 2);
        topLinks.forEach(link => {
          const linkText = link.label.length > 25 ? link.label.substring(0, 22) + "..." : link.label;
          doc.text(`  - ${linkText}: ${link.count}`, 24, tableY + 4);
          tableY += 5;
        });
        doc.setFontSize(10);
      }
