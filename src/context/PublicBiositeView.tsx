...
import type {
  ...
  AppLink,
  WhatsAppLink,
} from "../interfaces/PreviewContext";
...
const PublicBiositeView = () => {
  ...
  const [biositeData, setBiositeData] = useState<PublicBiositeData | null>(
    null
  );
  ...
  const handleEmbedClick = useCallback(
    async (embedId: string, embedType: "music" | "video" | "social-post") => {
  ...
  const getMusicEmbed = useCallback(() => {
    if (
      ...
      !Array.isArray(biositeData.biosite.links)
    )
      return null;

    // First try to find by link_type
    const musicByType = biositeData.biosite.links.find(
  ...
  const getSocialPost = useCallback(() => {
    ...
    // First try to find by link_type
    const socialPostByType = biositeData.biosite.links.find(
  ...
  const getVideoEmbed = useCallback(() => {
    ...
    // First try to find by link_type
    const videoByType = biositeData.biosite.links.find(
  ...
  const orderedContentSections = useMemo(() => {
    ...
    findPlatformForLink,
    filterRealSocialLinks,
    analytics.handleSocialLinkClick,
    analytics.trackLinkClick,
    analytics.handleLinkClick,
  ...
  const themeConfig = getThemeConfig(biositeData.biosite);
  const isExposedRoute = true;
  const validBackgroundImage = isValidImageUrl(
    biositeData.biosite?.backgroundImage
  )
    ? biositeData.biosite?.backgroundImage
    : null;
  const validAvatarImage = isValidImageUrl(biositeData.biosite?.avatarImage)
    ? biositeData.biosite?.avatarImage
    : null;
  const defaultAvatar =
  ...
  const description =
    biositeData.biosite.owner.description || user?.description;
