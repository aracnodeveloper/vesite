...
export const useLinkOperations = ({
    ...
    const toggleAdminLink = useCallback(async (linkId: string, isSelected: boolean) => {
        ...
        try {
            ...
            if (isSelected) {
                // Apply link to children - match the PathLinkDto structure exactly (no biositeId)
                const linkData = {
                    label: String(link.label || link.title || 'Link'),
                    url: String(link.url || ''),
                    icon: String(link.icon || 'link'),
                    orderIndex: parseInt(String(link.orderIndex || 0), 10),
                    link_type: String(link.link_type || LINK_TYPES.REGULAR)
                };

                // Validate that all required fields are present and valid
                if (!linkData.label) throw new Error('Label is required');
                if (!linkData.url) throw new Error('URL is required');
                if (!linkData.icon) throw new Error('Icon is required');
                if (!Number.isInteger(linkData.orderIndex)) throw new Error('OrderIndex must be integer');
    ...
    const updateAdminLink = useCallback(async (linkId: string, linkData: any) => {
        ...
        try {
            ...
            const updateData = {
                biositeId: biositeData.id, // Include biositeId as it seems to be required
